...
import {REPORTSAN, REPORTCARDS, REPORTANALYTICS} from "@/components/Constant/ShareData";
    ...
    <>
     ...
     <Box py={[5, 20]}>
      ...
      {REPORTCARDS.map((item, index) => {
        return(
        <Box key={index} boxShadow={'2xl'} p={5} bg={'gray.100'} _hover={{ bg: "blue.400", color: "white", cursor: "pointer" }}>
        ...
        <SimpleGrid columns={[1, 2]} spacing={3}>
          {REPORTANALYTICS.map((item, index) => (
            <> 
              ...
              <Box key={index} px={10} alignItems='center'>
                <Text py={8} fontSize={'lg'} fontWeight={'bold'} color={"red.500"}>Insights</Text>
                <Heading pb={15} color={'gray.700'} size={['md', '2xl']}>{item.heading}</Heading>
                <Text py={10} color={"grey.300"} fontSize="md">
                  {item.paragraph}
                </Text>
